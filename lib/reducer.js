...
function reduceAll(state, action) {
  ...
  var reducers = T.toObject($$fns);
  var t = state.$$tree;
  var entries = local ? [[ident, T.get(t, ident)]] : // localized actions trigger only the relevant reducer
  T.entries(state.$$tree);

  for (var i = 0; i < entries.length; i++) {
    var _entries$i = _slicedToArray(entries[i], 2);

    var key = _entries$i[0];
    var value = _entries$i[1];
